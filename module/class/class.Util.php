...
class Util
{
	...
	//마지막 글자($skip 수만큼) $suffix표시
	function NameStrLast($str, $skip, $suffix){
		$strlen = mb_strlen($str, 'UTF-8');
		$strTxt = '';

		//글자수가 skip보다 적을경우 전체 suffix표시
		if($strlen < $skip){
			for($i=0; $i<$strlen; $i++){
				$strTxt .= $suffix;
			}
			...
			$slen = $strlen - $skip;
			$strTxt = iconv_substr($str, 0, $slen, 'UTF-8');
	...
	//skip 수 만큼 suffix 치환
	function NameCutStr1($str, $skip, $suffix){ 
		$strlen = mb_strlen($str, 'UTF-8');
		$strTxt = '';
		...
		if($strlen < $skip){
			...
			$slen = $strlen - $skip;
			$strTxt .= iconv_substr($str, $skip, $slen, 'UTF-8');
		}
